...
async function takeScreenshotForVehicle(vehicle, hash, model) {
  ...
  let camPos = {
    x:
      center.x +
      (Math.max(modelSize.x, modelSize.y, modelSize.z) + 2) * Math.cos(340),
    y:
      center.y +
      (Math.max(modelSize.x, modelSize.y, modelSize.z) + 2) * Math.sin(340),
...
async function ResetPedComponents() {
  ...
  SetPedComponentVariation(ped, 6, 0, 0, 0); // Shoes
  SetPedComponentVariation(ped, 9, 0, 0, 0); // Armor
  SetPedComponentVariation(ped, 3, 15, 0, 0); // Torso (Set to 15 - common base)
  SetPedComponentVariation(ped, 8, 0, 0, 0); // Undershirt (Keep at 0 for now)
  SetPedComponentVariation(ped, 4, 0, 0, 0); // Legs
  SetPedComponentVariation(ped, 11, 15, 0, 0); // Top (Set to 15 - common base)
  SetPedHairColor(ped, 45, 15);

  ClearAllPedProps(); // Clear props after setting base components
...
async function LoadPropVariation(ped, component, prop, texture) {
  ...
  SetPedPropIndex(ped, component, prop, texture, true); // Changed last parameter to true to attach prop
...
RegisterCommand('screenshot', async (source, args) => {
  ...
  for (const modelHash of modelHashes) {
    if (IsModelValid(modelHash)) {
      ...
      await ResetPedComponents(); // Moved reset outside the loops
      await Delay(150);
      ...
      for (const type of Object.keys(config.cameraSettings)) {
        for (const stringComponent of Object.keys(
          ...
          // await ResetPedComponents(); // Removed reset from inside the loop
          // await Delay(150);
          const component = parseInt(stringComponent);
          if (type === 'CLOTHING') {
            // Clear props before processing clothing components to avoid conflicts
            if (
              component !== 1 &&
              component !== 3 &&
              component !== 4 &&
              ...
              // If not a standard clothing component, maybe clear props? Or handle specific cases.
              // For now, let's assume standard components are handled.
              // We might need finer control later.
            } else {
              // Reset specific interfering slots if needed, e.g., clear hats before masks
              if (component === 1) {
                // Mask
                ClearPedProp(ped, 0); // Clear hat
              }
              // Add other specific resets if known conflicts exist
            }
            ...
            // Ensure a visible base torso (component 3, drawable 0) is set before applying props
            SetPedComponentVariation(ped, 3, 0, 0, 0);
            await Delay(50); // Add a small delay after setting torso
            ...
            for (let prop = 0; prop < propVariationCount; prop++) {
              const textureVariationCount = GetNumberOfPedPropTextureVariations(
                ped,
                component,
                prop
              );
      ...
      SetModelAsNoLongerNeeded(modelHash); // Ensure model is released here too
    }
  }
  SetPedOnGround(); // Ensure ped is grounded after processing
  startWeatherResource(); // Restart weather sync
  SendNUIMessage({
    action: 'end',
  });
  DestroyAllCams(true);
...
RegisterCommand('customscreenshot', async (source, args) => {
  ...
  let drawable =
    args[1].toLowerCase() == 'all' ? args[1].toLowerCase() : parseInt(args[1]);
  let prop =
    args[1].toLowerCase() == 'all' ? args[1].toLowerCase() : parseInt(args[1]);
  ...
  for (const modelHash of modelHashes) {
    if (IsModelValid(modelHash)) {
      ...
      // ResetPedComponents(); // Reset is now done ONCE before the loop in the main 'screenshot' command
      // await Delay(150);

      if (drawable == 'all') {
        SendNUIMessage({
          action: 'start', // Keep start message here for 'all' case
        });
        if (type === 'CLOTHING') {
          const drawableVariationCount = GetNumberOfPedDrawableVariations(
            ped,
      ...
      FreezeEntityPosition(ped, false);
      clearInterval(interval);
      SetModelAsNoLongerNeeded(modelHash); // Ensure model is released
    }
  }
  SetPedOnGround(); // Ensure ped is grounded
  startWeatherResource(); // Restart weather sync
  SendNUIMessage({
    action: 'end', // Keep end message here for 'customscreenshot'
  });
  DestroyAllCams(true);
  DestroyCam(cam, true);
  RenderScriptCams(false, false, 0, true, false, 0);
...
RegisterCommand('screenshotvehicle', async (source, args) => {
  ...
  if (type === 'all') {
    ...
    for (const vehicleModel of vehicles) {
      ...
      SendNUIMessage({
        action: 'progressUpdate', // Update for vehicle progress
        data: {
          type: vehicleModel,
          value: vehicles.indexOf(vehicleModel) + 1,
          max: vehicles.length + 1, // Assuming length is correct max
        },
      });
    ...
    if (IsModelValid(vehicleHash)) {
      SendNUIMessage({
        action: 'progressUpdate', // Update for single vehicle progress
        data: {
          type: vehicleModel,
          value: 1, // Only one vehicle
          max: 1, // Only one vehicle
        },
      });
