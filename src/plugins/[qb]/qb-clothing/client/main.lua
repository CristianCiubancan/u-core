...
-- the qb-clothing:server:getLocale callback at openMenu time, and refreshed
-- by QBCore:Locale:Changed for mid-session swaps.
local currentLocale = nil

-- u-core: pause-menu/quit-warning suspension. Three-way coverage matches
-- qb-spawn / qb-multicharacter (project_nui_focus_suspension memory):
...
-- creatingCharacter doubles as our nuiOpen flag (set in openMenu/close).
...
-- plain data) and call replace here, where `Lang` resolves to qb-clothing's
-- instance. Same pattern as qb-spawn's applyLocaleHere.
...
local function relabelMenus(menus)
    ...
    for _, m in ipairs(menus) do
        if type(m) == 'table' and type(m.labelKey) == 'string' then
            m.label = Lang:t(m.labelKey)
        end
    end
end
...
local function openMenu(allowedMenus)
    ...
    -- own Lang BEFORE the open message, so the first translations dict
    -- ships in their language. The callback is fast — the server cache
    -- is warm by this point (PlayerLoaded fired during loadUserData
    -- earlier in the login flow).
    QBCore.Functions.TriggerCallback('qb-clothing:server:getLocale', function(code)
        if type(code) == 'string' then
            currentLocale = code
            applyLocaleHere(code)
        end
        sendOpen(allowedMenus, trackerMeta)
    end)
...
-- u-core: pause-menu / quit-warning suspension primitives. SendNUIMessage
-- is resource-scoped, so React listens for `setVisible` from this plugin
-- specifically (project_send_nui_message_scope).
local function isMenuOrWarningActive()
    return IsPauseMenuActive() or IsWarningMessageActive()
end

local function suspendUI()
    if not creatingCharacter or nuiHidden then return end
    nuiHidden = true
...
local function resumeUI()
    if not creatingCharacter or not nuiHidden then return end
    nuiHidden = false
    SetNuiFocus(true, true)
...
CreateThread(function()
    -- 100ms is fast enough that resume-on-close feels instant; idle cost
    -- (the common case — creatingCharacter false) is negligible.
    while true do
        Wait(100)
...
local function getOutfits(gradeLevel, data)
    ...
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            ...
            {menu = "accessoires", labelKey = "outfits.accessoires", label = Lang:t("outfits.accessoires"), selected = false}
        })
    end)
end
...
RegisterNUICallback('uiFocused', function(_, cb)
    ...
    -- ALT+TABs back, don't grab focus from it just because OS-level
    -- focus returned to the game window.
    if creatingCharacter and not nuiHidden and not isMenuOrWarningActive() then
        SetNuiFocus(true, true)
    end
    cb('ok')
end)
...
-- u-core: hot-reload safety. pnpm dev rebuilds + restarts this resource
-- without going through the menu close path; if the player is mid-edit
-- when the swap happens, NUI focus would stay grabbed and cursor would
-- be pinned. Release everything on stop so the swapped-in copy starts
-- from a clean state.
